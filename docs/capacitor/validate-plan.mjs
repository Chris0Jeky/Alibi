...
const requiredGateOwners = new Map([
  ['identity', 123],
  ['runtime', 126],
  ['migration', 128],
  ['physical-quality', 131],
  ['security', 132],
  ['delivery', 134],
  ['play-approval', 135],
]);
const requiredCascadeBoundary = {
  pullRequests: [115, 117],
  issue: 119,
  domain: 'cascade',
  inAuditedMain: true,
  nativeTransfer: 'excluded-until-CAP-05-CAP-06',
};
...
export function validatePlan(plan, config, { release = false } = {}) {
  ...
  if (!object(plan) || !object(config))
    return { errors: ['Plan and config must be JSON objects.'] };
  ...
  need(
    plan.nativeImplementationIncluded === false,
    'Planning must not claim an implemented native application.',
  );
  ...
  for (const key of [
    'androidServiceWorker',
    'remoteMainDocument',
    'otaExecutableUpdates',
    'nativeTelemetry',
  ])
    need(architecture[key] === false, `Unsafe or missing architecture policy: ${key}.`);
  need(
    architecture.androidOrigin === 'https://localhost',
    'Native origin must stay explicit and stable.',
  );
  need(
    Array.isArray(architecture.targets) && architecture.targets.join(',') === 'web,android',
    'Both targets must be retained.',
  );
  ...
  need(
    JSON.stringify(architecture.preservedWebOrigins) === JSON.stringify(expectedOrigins),
    'Both existing web origins must be preserved.',
  );

  const packages = Array.isArray(plan.workPackages) ? plan.workPackages : [];
  need(
    packages.length === requiredWorkPackages.size,
    'Work-package inventory must contain exactly CAP-01 through CAP-14.',
  );
  const byId = new Map();
  const issues = new Set();
  for (const item of packages) {
    if (!object(item)) {
      errors.push('Work package must be an object.');
      continue;
    }
    ...
    if (Array.isArray(item.dependsOn)) {
      need(
        ...
        `Repeated dependency for ${item.id}.`,
      );
      for (const dependency of item.dependsOn)
        need(typeof dependency === 'string', `Non-string dependency for ${item.id}.`);
    }
  }
  for (const [id, issue] of requiredWorkPackages) {
    const item = byId.get(id);
    need(!!item, `Missing required work package: ${id}.`);
    if (item) need(item.issue === issue, `Unexpected issue mapping for ${id}; expected #${issue}.`);
  }
  ...
  need(
    domains.length === requiredDomains.length,
  ...
  need(
    domains.every(
      (d) => object(d) && typeof d.source === 'string' && typeof d.database === 'string',
    ),
    'Each domain needs its source and database identity.',
  );

  const boundaries = Array.isArray(plan.integratedBoundary) ? plan.integratedBoundary : [];
  const cascade = boundaries[0];
  need(
    boundaries.length === 1 &&
      object(cascade) &&
  ...
  for (const gate of gates) {
    need(
      ...
      `Planning cannot manufacture acceptance: ${gate?.id}.`,
    );
    need(issues.has(gate?.ownerIssue), `Gate has no mapped owner issue: ${gate?.id}.`);
    need(
      gate?.ownerIssue === requiredGateOwners.get(gate?.id),
      `Unexpected owner issue for release gate: ${gate?.id}.`,
    );
  }
  need(
    object(plan.identity) && Object.values(plan.identity).every((v) => v === null),
    'Unapproved publisher/signing identity must remain unresolved.',
  );
  need(
    Array.isArray(plan.deliverables) &&
      new Set(plan.deliverables).size === plan.deliverables.length,
    'Deliverables must be an explicit unique list.',
  );
  for (const item of packages)
    if (object(item))
      need(
        plan.deliverables?.includes(item.document),
        `Work-package document is not inventoried: ${item.id}.`,
      );

  const server = object(config.server) ? config.server : {};
  const android = object(config.android) ? config.android : {};
  need(
    config.appId === 'example.unapproved.alibi.preview',
    'Example must use the non-publishable placeholder identity.',
  );
  ...
  need(
    server.hostname === 'localhost' && server.androidScheme === 'https',
    'Unsafe native scheme or hostname.',
  );
  ...
  need(
    !android.loggingBehavior || ['debug', 'none'].includes(android.loggingBehavior),
    'Android logging override is unsafe.',
  );
  need(
    Number.isSafeInteger(android.minWebViewVersion) && android.minWebViewVersion >= 120,
    'Example runtime floor must be explicit; support still needs device proof.',
  );
  need(
    config.plugins?.SystemBars?.insetsHandling === 'css',
    'SystemBars inset ownership must be explicit.',
  );
  if (release) {
    errors.push(
      'Release denied: this is a documentation-only planning checker, not a native artifact approval pipeline.',
    );
    for (const gate of gates) errors.push(`Unresolved release evidence: ${gate?.id}.`);
  }
  return {
    errors,
    workPackages: packages.length,
...
export function checkFiles(plan, directory = HERE) {
  ...
  function requireFile(relative, boundary, from = base) {
    if (typeof relative !== 'string' || !relative || path.isAbsolute(relative)) {
      errors.push(`Invalid relative file path: ${relative}.`);
      return;
    }
    const resolved = path.resolve(from, relative);
    if (!inside(boundary, resolved)) {
      errors.push(`Path escapes allowed root: ${relative}.`);
      return;
    }
    try {
      const real = fs.realpathSync(resolved);
      if (!inside(boundary, real) || !fs.statSync(real).isFile())
        errors.push(`Invalid file or symlink: ${relative}.`);
    } catch {
  ...
  for (const file of plan.deliverables || []) {
    ...
    for (const match of text.matchAll(/\[[^\]]*\]\(([^\s)]+)\)/g)) {
      ...
      if (/^[a-z][a-z0-9+.-]*:/i.test(link)) {
        errors.push(`Unexpected link scheme in ${file}: ${link}.`);
        continue;
      }
  ...
  for (const item of plan.workPackages || [])
    if (!roadmap.includes(`https://github.com/${plan.repository}/issues/${item.issue}`))
      errors.push(`Roadmap misses issue ${item.issue}.`);
  return errors;
}
...
export function inspectDirectory(directory = HERE, options = {}) {
  try {
    ...
    const config = JSON.parse(
      fs.readFileSync(path.join(directory, 'examples/capacitor.config.json'), 'utf8'),
    );
