...
**Architecture proposal · 11 September 2026 · non-publishable Android preview added in CAP04.**
...
Do not copy the sample applicationId into a production project. The CAP04 preview now includes root
Capacitor configuration, a generated Android source project and local debug signing. It does not
include production identity, signing custody, an active publication workflow or account
provisioning. The planning checker remains a design gate, separate from the executable preview.
...
publication acceptance. Tracked Gradle lockfiles cover the app and Capacitor subprojects, including
the Cordova project regenerated by sync; SHA-256 verification metadata records resolved artifacts.
Only regenerate both sets deliberately when updating native dependencies. Splash failure recovery
remains open under #126. The pinned
