...
test('planning checker never authorizes a native release', () => {
  ...
  for (const gate of originalPlan.releaseGates)
    assert.ok(
      result.errors.some((error) => error.includes(`Unresolved release evidence: ${gate.id}`)),
    );
});

test('duplicate work-package identifiers are rejected', () => {
  rejects((plan) => {
    plan.workPackages[1].id = plan.workPackages[0].id;
  }, /Duplicate work-package/);
});

test('duplicate GitHub issue mappings are rejected', () => {
  rejects((plan) => {
    plan.workPackages[1].issue = plan.workPackages[0].issue;
  }, /Duplicate issue/);
});

test('work-package inventory keeps exact IDs and issue mappings', () => {
  rejects((plan) => {
    const first = plan.workPackages[0].issue;
    plan.workPackages[0].issue = plan.workPackages[1].issue;
    plan.workPackages[1].issue = first;
  }, /Unexpected issue mapping/);
  rejects((plan) => {
    plan.workPackages = plan.workPackages.filter((item) => item.id !== 'CAP-14');
    plan.workPackages.find((item) => item.id === 'CAP-13').dependsOn = plan.workPackages
      .find((item) => item.id === 'CAP-13')
      .dependsOn.filter((id) => id !== 'CAP-14');
  }, /exactly CAP-01 through CAP-14|Missing required work package: CAP-14/);
});

test('dependency cycles are rejected', () => {
  rejects((plan) => {
    plan.workPackages[1].dependsOn = ['CAP-04'];
  }, /Dependency cycle/);
});

test('unknown dependency identifiers are rejected', () => {
  rejects((plan) => {
    plan.workPackages[1].dependsOn = ['CAP-99'];
  }, /Unknown dependency/);
});

test('omitting a save domain is rejected', () => {
  rejects((plan) => {
    plan.saveDomains.pop();
  }, /five audited|Missing or repeated domain/);
});

test('inventing combined-backup coverage for challenges is rejected', () => {
  rejects((plan) => {
    plan.saveDomains.find((d) => d.id === 'challenges').combinedBackup = true;
  }, /Do not invent/);
});

test('integrated Cascade boundary is explicit and preserved', () => {
  rejects((plan) => {
    plan.integratedBoundary[0].pullRequests.reverse();
  }, /Integrated Cascade boundary/);
  rejects((plan) => {
    plan.integratedBoundary[0].issue = 120;
  }, /Integrated Cascade boundary/);
  rejects((plan) => {
    plan.integratedBoundary[0].inAuditedMain = false;
  }, /Integrated Cascade boundary/);
  rejects((plan) => {
    plan.integratedBoundary[0].nativeTransfer = 'included';
  }, /Integrated Cascade boundary/);
});

test('integrated Cascade boundary ignores JSON property order', () => {
  const plan = copy(originalPlan);
  const config = copy(originalConfig);
...
test('unapproved release and fabricated acceptance claims are rejected', () => {
  rejects((plan) => {
    plan.productionApproved = true;
  }, /production approval/);
  rejects((plan) => {
    plan.releaseGates[0].status = 'passed';
  }, /manufacture acceptance/);
  rejects((plan) => {
    plan.identity.publisher = 'Unapproved publisher';
  }, /remain unresolved/);
});

test('release gates keep their exact owner issues', () => {
  rejects((plan) => {
    const first = plan.releaseGates[0].ownerIssue;
    plan.releaseGates[0].ownerIssue = plan.releaseGates[1].ownerIssue;
    plan.releaseGates[1].ownerIssue = first;
  }, /Unexpected owner issue for release gate/);
});

test('both existing web origins must remain represented', () => {
  rejects((plan) => {
    plan.architecture.preservedWebOrigins.pop();
  }, /Both existing web origins/);
});

test('native service-worker and OTA-code shortcuts are rejected', () => {
  rejects((plan) => {
    plan.architecture.androidServiceWorker = true;
  }, /androidServiceWorker/);
  rejects((plan) => {
    plan.architecture.otaExecutableUpdates = true;
...
test('unsafe logging overrides and an implicit runtime floor are rejected', () => {
  rejects((plan, config) => {
    config.android.loggingBehavior = 'production';
  }, /logging override/);
  rejects((plan, config) => {
    delete config.android.minWebViewVersion;
  }, /runtime floor/);
});
...
test('malformed top-level JSON shapes and missing directories fail explicitly', () => {
  ...
  assert.match(
    inspectDirectory(path.join(directory, 'does-not-exist')).errors.join('\n'),
    /Cannot validate/,
  );
});
