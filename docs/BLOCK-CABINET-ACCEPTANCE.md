...
- The final integration fix at `9abfa3b` builds as `9245cce18870`, passes the formatting gate and keeps the optional pack below its 64 KiB cap. It includes the protected-import race fix from parent commit `9e69a0a`; the production bundle uses a delivered replay-worker asset, the standalone bundle embeds the same worker source, Classic and Cascade preserve the local motion choice while updating external motion floors, Zen hides the enhanced aside, the Cascade dialog is labelled by its rendered heading, and Undo/Redo retain usable keyboard focus.

## Production integration

The stacked integration layer adds actual-control tests for original Classic replay preservation, undo/redo, mouse drag, touch cancellation, keyboard focus, Cascade separation and persistence, stale-tab rejection, fallback controls, offline pack loading, route disposal, Zen presentation and responsive layout. The reviewed local matrix passes 71 checks at 390 and 1280 CSS pixels; its own workflow/check results still take precedence over this architecture note.
