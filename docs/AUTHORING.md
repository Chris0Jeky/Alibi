...
A local import changes one browser, not the website. To publish an official puzzle, add its validated definition to `content/catalog.json`, preserve all published IDs/revisions, run build/tests, and upload the entire release to the existing production project. `npm run generate` replays the seeded additions while preserving every definition, array position and top-level header field already present in the checked-in catalogue; only seed IDs absent from that array are appended. It is a development tool, not the normal publishing command. Do not run it over a hand-edited published catalogue casually.

Casebooks live in `content/casebooks.json`. Each has id/title/tagline/setting/intro/ending/color/icon, a `format` of `continuous` or `anthology`, and chapters with `{id,name,brief}`. A continuous casebook may also provide `cast`, chapter `time`/`revelation`, and `artwork`; keep those fields consistent with the authored chronology. Chapter IDs reference catalogue puzzle IDs. All chapters are visible from the casebook page; continuous casebooks suggest the next chapter after completion, while anthology chapters remain open in any order and every chapter remains available for replay. Both formats share puzzle completion with the main library. Bridges chapters use the same published `bridges` definitions as the collection. Bellweather is the current continuous casebook; Briar House, Night Train and Glasshouse are standalone anthologies. Do not claim that incidental story names from separate puzzles form a tightly continuous plot. Generation preserves same-ID checked-in casebooks, including editorial corrections and array order, retains added books missing from its seeds, and appends only seeded books absent from the checked-in list.
