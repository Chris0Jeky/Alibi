...
(function (G) {
  ...
  function bar(quiet = false) {
    return `<aside class="theatre-rail" aria-label="Room atmosphere"><span class="theatre-rail-title">${emblem(scene?.motif)}<span>${escape(scene?.title || 'The club')}<small>AN IMAGINED PLACE · YOUR OWN PACE</small></span></span>${quiet ? '' : '<details class="theatre-settings" data-disclosure-key="room-settings"><summary id="room-settings-summary">Room settings</summary>'}<div class="theatre-controls">${quiet ? '<a id="quiet-room-choice" href="#/home">Choose a room ↗</a>' : `<button type="button" data-theatre-sound aria-pressed="${sound}">Room sound ${sound ? 'on' : 'off'}</button><button type="button" data-theatre-motion aria-pressed="${movement}">${movement ? 'Still the room' : 'Let it breathe'}</button><button type="button" data-theatre-data aria-pressed="${G.AlibiDelivery?.mode() === 'local'}">${G.AlibiDelivery?.mode() === 'local' ? 'Painted edition' : 'Rich edition'}</button>`}</div>${quiet ? '' : '</details>'}</aside>`;
  }
  function room(story) {
