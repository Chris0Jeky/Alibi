...
(function (G) {
  ...
  function collectionPicker(selected = '') {
    ...
    return `<section class="collection-scope" aria-label="Current collection"><span>${active ? `Showing: <strong>${esc(active.title)}</strong>` : 'All collections · original puzzles included'}</span>${active ? '<button class="btn secondary small" data-action="curation-venue" data-value="">Show all collections</button>' : ''}</section><details class="curation-collections" data-disclosure-key="collections"><summary>Choose a setting <span class="disclosure-hint">Four illustrated collections</span></summary><p class="collection-intro">A harbour observatory, a rain-soaked glasshouse, a gallery after dark, or the last post before snow. Where will your next puzzle take you?</p><button class="btn secondary small" data-action="curation-venue" data-value="" aria-pressed="${!selected}">All collections</button>${collectionCards(selected)}<p class="collection-caption">Each collection brings together ${config.collections[0].puzzleIds.length} standalone puzzles across thirteen games. Pick any record and play at your own pace.</p>${gallery(selected)}</details>`;
  }
  function collectionCards(selected = '', discover = false) {
    return `<div class="collection-cards">${config.collections.map((c) => `<button class="collection-card" data-action="${discover ? 'discover-collection' : 'curation-venue'}" data-value="${esc(c.id)}" ${discover ? '' : `aria-pressed="${selected === c.id}"`}><img src="${esc(G.ALIBI_CURATION_MEDIA['cover-' + c.id])}" alt="" width="640" height="400" loading="lazy" decoding="async"><span class="collection-card-copy"><small class="eyebrow">COLLECTION ${esc(c.number)} · ${c.puzzleIds.length} PUZZLES</small><strong>${esc(c.title)}</strong><span>${esc(c.line)}</span><b>${selected === c.id && !discover ? 'Selected collection ✓' : 'Explore collection →'}</b></span></button>`).join('')}</div>`;
  }
  function news() {
