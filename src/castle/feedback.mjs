...
export function attachFeedback(root, environment = {}) {
  ...
  const Audio = environment.AudioContext || globalThis.AudioContext || globalThis.webkitAudioContext;
  const abort = new AbortController();
  const voices = new Set(), animations = new Set();
  let context = null, generation = 0, disposed = false;
  ...
  async function cue(event) {
    ...
    if (root.host.dataset.reduced !== 'true') {
      const animation = root.querySelector('#board')?.animate?.(
        [{ transform: 'translateY(1px)' }, { transform: 'translateY(0)' }],
        { duration: kind === 'complete' ? 220 : 130, easing: 'ease-out' },
      );
      if (animation) {
    ...
    try {
      ...
      voices.add(tone); tone.start(now); tone.stop(now + 0.11);
    } catch { stopSound(); }
  }
