...
const roomScenes = {
  gatehouse: 'briar-house',
  library: 'reading-room',
  observatory: 'bellweather',
  cartography: 'cartographer',
  orangery: 'glasshouse',
  museum: 'reading-room',
  workshop: 'briar-house',
  study: 'briar-house',
  'west-stair': 'briar-house',
...
function painting(sceneId) {
  ...
  const source =
    scene &&
    (scene.curation ? globalThis.ALIBI_CURATION_MEDIA : globalThis.ALIBI_MEDIA)?.[scene.art];
...
function image(id, description) {
  ...
  return source
    ? `<img class="castle-painting" src="${escape(source)}" alt="${escape(description)}" decoding="async">`
    : '<div class="castle-painting castle-paper" aria-hidden="true"></div>';
}
...
function interior(room) {
  return image(
    roomScenes[room.id],
    `${room.name}: a reused Alibi atmosphere painting. Puzzle clues appear in the text and controls.`,
  );
}
