export function createPackCache(getConfig, storage = globalThis.caches) {
  ...
  async function fill() {
    ...
    const prefix = 'alibi-castle-pack-', name = prefix + c.build;
    try {
      const cache = await storage.open(name);
      if (!await cache.match(expected)) {
        try {
          await cache.addAll([new Request(new URL(expected, globalThis.location.href), {
            cache: 'reload', signal: AbortSignal.timeout(15000),
          })]);
        } catch (error) {
  ...
  return {
    load() {
      if (!pending) pending = fill().finally(() => { pending = null; });
      return pending;
    },
    available: () => ready,
  };
