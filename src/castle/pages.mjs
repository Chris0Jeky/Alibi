...
export function createPages({state, view, selected, era, search, filter}) {
  ...
  function roomCard(r) {
    const status=E.roomStatus(state,r), done=E.has(state,r.puzzle);
  ...
  function rail(r) {
    const status=E.roomStatus(state,r), completed=E.has(state,r.puzzle);
    let action=button('Enter room','visit',r.id,'class="primary"');
    if(view==='room'&&status.open)action=r.puzzle==='rest'?button('Open the guest book','rest','','class="primary"'):r.puzzle==='reveal'?button('Read the margin','resolution','','class="primary"'):button(completed?'Revisit the puzzle':'Inspect the puzzle','puzzle',r.puzzle,'class="primary"');
  ...
  function mapPage() {
    const visible=W.rooms.filter(r=>r.implemented&&(r.id!=='west-stair'||E.has(state,'inference')));
  ...
  function roomPage() {
    const r=room();
    return `<div class="layout"><section class="scene transition" aria-label="${escape(r.name)} interior">${Art.interior(r,era)}<div class="scene-title"><span class="eyebrow">${escape(r.wing)}</span><h1>${escape(r.name)}</h1></div><div class="scene-controls">${link('Return to the grounds','map')}${button('Notebook','notebook')}</div></section>${rail(r)}</div><section class="castle-objects" aria-label="Room details"><p>On a closer look<small>Optional observations. No points or hidden timer.</small></p>${roomObjects(r.id).map(object=>button(escape(object.title),'object',object.id)).join('')}</section>`;
  }
  function museumPage() {
  ...
  function notebook() {
    return `<section class="page"><div class="row"><h1>Castle notebook</h1>${button('Export castle save','export')}</div><p class="small">This export contains Wrenmere Chapter I only. It does not include the Cabinet, Club, challenge, garden, companion or realm saves. Castle import is not available in this first integration.</p><label for="notes">Your notes</label><textarea id="notes" maxlength="12000" placeholder="What have you established? What still needs checking?">${escape(state.notes)}</textarea><p class="small" id="notes-count">${state.notes.length} / 12,000 characters</p><h2>Collected records</h2><div class="directory">${state.preferences.story?E.evidence(state).map(x=>`<article class="card evidence"><small>${escape(x.kind)} · ${escape(x.from)}</small><h3>${escape(x.title)}</h3><p>${escape(x.text)}</p><p><em>${escape(x.question)}</em></p></article>`).join('')||'<p>The library is a useful place to start looking.</p>':'<p>Story records are hidden. Turn the story back on in Preferences to read them.</p>'}</div></section>`;
  }
  function directory() {
    const rooms=W.rooms.filter(r=>(filter==='all'||r.wing===filter)&&`${r.name} ${r.line}`.toLowerCase().includes(search.toLowerCase()));
