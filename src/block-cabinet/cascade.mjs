...
export function shape(id, rotation = 0) {
  if (!integer(id, 0, SHAPES.length - 1) || !integer(rotation, 0, 3))
    throw Error('Unknown shape or rotation.');
...
export function initial(seed = 'ATELIER-01') {
  ...
  [33, 38, 40, 45, 50, 55, 59, 62].forEach((i) => {
    board[i] = 2;
  });
  return {
    rules: RULES,
...
export function legal(s, slot, cell, rotation = 0) {
  ...
  return shape(s.tray[slot], rotation).cells.every(
    ([dx, dy]) => x + dx < 8 && y + dy < 8 && !s.board[(y + dy) * 8 + x + dx],
  );
}
...
export function resolve(board) {
  ...
  for (let depth = 1; depth <= 8; depth++) {
    ...
    const cells = Array.from({ length: 64 }, (_, i) => i).filter(
      (i) => rows.includes(Math.floor(i / 8)) || columns.includes(i % 8),
    );
...
export function move(s, slot, cell, rotation = 0) {
  ...
  if (tray.every((p) => p === null)) {
    round++;
    tray.splice(0, 3, ...deal(s.seed, round));
  }
  ...
  const q = {
    ...
    round,
    turn: s.turn + 1,
    score: s.score + placed.length + resolved.bonus,
    charges: Math.min(3, s.charges - (rotation ? 1 : 0) + lines),
    relics: s.relics + resolved.relics,
    waves: resolved.waves,
    lastClear: resolved.waves[0] || { rows: [], columns: [] },
    done: false,
    won: false,
  };
...
export function record(seed = 'ATELIER-01') {
  return { rules: RULES, seed: seedText(seed), log: [], redo: [] };
}
export function replay(value) {
  if (
    !value ||
    Object.keys(value).sort().join(',') !== 'log,redo,rules,seed' ||
    value.rules !== RULES ||
    !Array.isArray(value.log) ||
    !Array.isArray(value.redo) ||
    value.log.length + value.redo.length > 35
  )
    throw Error('Unsupported or oversized Cascade replay.');
  const apply = (s, a) => {
    if (
      !a ||
      Object.keys(a).sort().join(',') !== 'cell,rotation,slot' ||
      !integer(a.rotation, 0, 3)
    )
      throw Error('Invalid Cascade move.');
    return move(s, a.slot, a.cell, a.rotation);
  };
  let s = initial(value.seed);
  for (const a of value.log) s = apply(s, a);
  let future = s;
  for (const a of [...value.redo].reverse()) future = apply(future, a);
  return s;
}
