...
(function (G) {
  ...
  const rooms = [
    {
      ...
      title: 'The Study',
      mark: '01',
      detail: 'East-facing window. A clock is ticking above the desk.',
      fact: 'East window · working clock',
      family: 'scene',
    },
    {
      id: 'library',
      title: 'The Library',
      mark: '02',
      detail: 'West-facing window. There is no clock among the shelves.',
      fact: 'West window · no clock',
      family: 'nonogram',
    },
    {
      id: 'maps',
      title: 'The Map Room',
      mark: '03',
      detail: 'East-facing window. A pale circle marks where the clock used to hang.',
      fact: 'East window · clock removed',
      family: 'bridges',
    },
  ];
  ...
  function catalogue(puzzles, records, filters, names = {}) {
    ...
    return puzzles.filter((p) => {
      ...
      return (
        (!filters.family || p.type === filters.family) &&
        (!filters.level || p.difficulty === filters.level) &&
        (!filters.progress || filters.progress === status) &&
        (!query ||
          [p.title, p.subtitle, p.type, names[p.type], p.collection]
            .join(' ')
            .toLocaleLowerCase()
            .includes(query))
      );
  ...
  function recent(records) {
    return records
      .filter((r) => r.moves > 0 && !completed(r))
      .slice()
      .sort(
        (a, b) =>
  ...
  function firstPuzzle(puzzles, records) {
    const saved = new Map(records.map((r) => [r.key, r]));
    return (
      puzzles.find((p) => p.id === 'binary-01' && !completed(saved.get(key(p)))) ||
      puzzles.find((p) => p.difficulty === 'Gentle' && !completed(saved.get(key(p)))) ||
      puzzles[0]
    );
  }
  function gameRuns(clubState, E) {
    ...
    const games = {
      ...
      duel: [
        'Lantern Duel',
        (r) => r.log.reduce((s, i) => E.reversi.move(s, i), E.reversi.initial()),
      ],
      archive: [
        'Archive Heist',
        (r) => r.log.reduce((s, i) => E.warehouse.move(s, i), E.warehouse.initial(r.level)),
      ],
    };
  ...
  function reduce(state, action) {
    ...
    if (
      action.type === 'answer' &&
      rooms.some((r) => r.id === action.id) &&
      state.letter &&
      state.visited.length === rooms.length
    ) {
