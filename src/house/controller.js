...
(function (G) {
  ...
  function create(bridge, club) {
    ...
    const refresh = (focus) => {
      bridge.render();
      (document.getElementById(focus) || document.getElementById('main'))?.focus({ preventScroll: true });
    };
    function snapshot() {
      ...
      return {
        ...
        route, puzzles, records, study, limit,
        ...
        icons: { scene: '♟', dossier: '▤', witness: '◈', binary: '☾', nonogram: '▦', bridges: '⌁', lightup: '☼', sudoku: '9' },
      };
    }
    function showObservation(title, text, source) {
      modalOrigin = { hash: location.hash, id: source.id };
      bridge.dialog(title, `<div class="hx-modal">${source.dataset.room ? G.AlibiHouseComponents.engraving(source.dataset.room) : G.AlibiHouseComponents.icon('mail')}<p>${V.esc(text)}</p><p class="hx-footnote">Added to the desk study in your notebook. This study is session-only.</p></div>`, [{ label: 'Back to the desk study', action: 'close-dialog', secondary: true }], 'hx-sheet');
    }
    document.addEventListener('click', (event) => {
      ...
      if (action === 'filters') {
        ...
        document.querySelectorAll('#hx-filter-form select').forEach(el => { el.value = ''; });
        document.querySelector('#hx-filter-form select')?.focus();
      } else if (action === 'play') {
        const key = button.dataset.key;
        const record = bridge.records().find((r) => r.key === key);
        const puzzle = record?.puzzle || bridge.all().find((p) => M.key(p) === key);
        if (!puzzle) return bridge.toast('That puzzle is no longer available. Your existing saves are unchanged.', true);
        returnTo = location.hash;
        ...
        showObservation('The envelope', '“Look east for the room that has stopped keeping time.” A small key is mentioned in the margin. Inspect the rooms before deciding where it was left.', button);
    ...
    document.addEventListener('submit', (event) => {
      ...
      if (['hx-search-form', 'hx-filter-form'].includes(form.id)) {
        ...
        const hash = M.url({ view: 'puzzles', ...Object.fromEntries(['q', 'family', 'progress', 'level'].map((field) => [field, String(data.get(field) || '').trim()])) });
        ...
        refresh('hx-answer');
        if (study.solved) bridge.toast('Desk study complete. The east window and the removed clock point to the Map Room.');
      }
    }, options);
    ...
    function keepFocusVisible() {
      ...
      visibilityFrame = requestAnimationFrame(() => {
        const el = document.activeElement, dock = document.querySelector('.hx-mobile');
        if (!active() || !el?.closest('.hx-content') || !dock || getComputedStyle(dock).position !== 'fixed') return;
        const r = el.getBoundingClientRect(), edge = dock.getBoundingClientRect().top;
        if (r.bottom > edge - 12) window.scrollBy({ top: r.bottom - edge + 16, behavior: 'instant' });
      });
    }
    function keyboardInset() {
      const vv = G.visualViewport;
      const editing = document.activeElement?.matches('.hx-experience input:not([type=checkbox]), .hx-experience select, #dialog.hx-sheet input, #dialog.hx-sheet select');
      document.body.dataset.houseKeyboard = active() && !!editing && !!vv && vv.height < innerHeight * 0.75 ? 'true' : 'false';
    }
    document.addEventListener('focusin', () => { keepFocusVisible(); keyboardInset(); }, options);
    ...
    document.addEventListener('error', (event) => {
      if (event.target.matches?.('img[data-house-art]')) event.target.classList.add('hx-art-unavailable');
    }, { ...options, capture: true });
    return {
      home() {
      ...
      afterRender(route) {
        ...
        if (active()) {
          if (postRouteFocus) {
            const id = postRouteFocus, hash = location.hash;
            postRouteFocus = '';
            requestAnimationFrame(() => { if (active() && location.hash === hash) document.getElementById(id)?.focus(); });
          }
          document.querySelectorAll('.hx-experience [data-house-action], .hx-experience a[href^="#/salon/"]').forEach((el, i) => {
            if (!el.id) el.id = `hx-action-${el.dataset.houseAction || 'game'}-${el.dataset.room || el.dataset.key || 'control'}-${i}`;
          });
          if (returnTo === location.hash && returnFocus) {
            const hash = returnTo, id = returnFocus;
