...
(function (G) {
    ...
    await new Promise((resolve, reject) => {
      ...
      const finish = (ok) => {
        ...
        if (ok) resolve();
          ...
          el.remove();
          reject(Error('Preview stylesheet unavailable.'));
        }
      };
  ...
  function offlineLabel() {
    ...
    if (el)
      el.textContent = h.offline
        ? 'Preview files cached for offline use'
        : 'Offline preview copy not ready';
  }
  async function cachePack() {
    if (!cfg.files || !G.caches) return;
    const prefix = 'alibi-house-pack-',
      name = prefix + cfg.build;
    try {
