...
function sources(root) {
  ...
  return { source: ['model.js', 'components.js', 'view.js', 'controller.js'].map(read).join('\n'), cssSource: read('style.css') };
}
function build(root, dist) {
  ...
  return { config: { script: emit(source, 'js'), css: emit(cssSource, 'css') }, standalone: { source, cssSource }, bytes: Buffer.byteLength(source) + Buffer.byteLength(cssSource), scriptGzipBytes: zlib.gzipSync(source).length, cssGzipBytes: zlib.gzipSync(cssSource).length };
}
