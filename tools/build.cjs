...
self.addEventListener('fetch',event=>{const r=event.request,u=new URL(r.url);if(r.method!=='GET'||u.origin!==self.location.origin||(u.pathname.endsWith('/sw.js')||u.pathname.startsWith('/api/')))return;event.respondWith((async()=>{const c=await caches.open(CACHE);if(u.pathname===new URL('./quiet-wing-sources.html',self.registration.scope).pathname)return await caches.match(r)||fetch(r);if(r.mode==='navigate')return await c.match(new URL('./',self.registration.scope).href)||fetch(r);const hit=await c.match(r);if(hit)return hit;if(u.pathname.includes('/assets/')){const prior=await caches.match(r);if(prior)return prior;}return fetch(r);})());});
`;
