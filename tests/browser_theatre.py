...
with sync_playwright() as pw:
 ...
 for width in (390,1280):
  ...
  page.on('request',lambda r:media.append(r.url) if '.mp4' in r.url or '.ogg' in r.url or '.mp3' in r.url else None)
  ...
  page.wait_for_function('()=>!AlibiTheatre.diagnostics().sound')
  check(not page.evaluate('AlibiTheatre.diagnostics().localAudio'),'Uncached offline playback stays silent without synthetic fallback')
  c.set_offline(False)
  page.locator('[data-theatre-sound]').click()
  page.wait_for_function('()=>document.querySelector("[data-theatre-sound-status]").textContent.startsWith("Playing")')
  page.wait_for_function('async()=>{const c=await caches.open("alibi-ambience-v1");return (await c.keys()).length===1}')
  check(page.evaluate('AlibiTheatre.diagnostics().streamingAudio'),'Recorded rain plays after explicit opt-in')
  page.locator('[data-theatre-volume]').fill('10')
  check(page.evaluate('AlibiTheatre.diagnostics().volume')==0.1,'Volume control changes recording volume')
  ...
  page.locator('[data-theatre-sound]').click()
  page.wait_for_function('()=>document.querySelector("[data-theatre-sound-status]").textContent.includes("Playing beach waves")')
  check(True,'Previously downloaded waves play offline')
