    ...
    try:
        with sync_playwright() as p:
            ...
            for width in [390,1280]:
                ...
                markers=page.locator('.hotspot.observation')
                toggle=page.get_by_label('Show inspectable objects')
                text_control=page.get_by_role('button',name='A pencilled correction',exact=True)
                expect(toggle).to_be_checked()
                expect(markers).to_be_visible()
                expect(text_control).to_be_visible()
                toggle.uncheck()
                ...
                for _ in range(2):
                    ...
                    expect(text_control).to_be_focused()
                toggle.check()
                report['checks'].append(f'{width}: inspectable markers can hide without removing text controls; Enter, Escape and focus return survive rapid use')
