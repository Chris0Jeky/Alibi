...
const results = [];
function solvedState(p) {
  const E = C.registry[p.type];
  let s = E.initial(p);
  const apply = (a) => (s = E.reduce(p, s, a));
  if (p.type === 'scene') {
    for (const person of p.people)
      apply({ type: 'place', who: person.id, cell: p.solution[person.id] });
    ok(!E.complete(p, s), p.id + ' requires final accusation');
    apply({ type: 'accuse', who: C.murderer(p, s) });
  } else if (p.type === 'dossier') {
    for (let cat = 0; cat < 2; cat++)
      for (let r = 0; r < p.size; r++)
        apply({
          type: 'mark',
          cell: cat * p.size ** 2 + r * p.size + p.solution[cat * p.size + r],
          value: 1,
        });
    ok(!E.complete(p, s), p.id + ' requires final accusation');
    apply({ type: 'accuse', who: p.solution.slice(p.size).indexOf(p.targetItem) });
  } else if (p.type === 'witness') apply({ type: 'accuse', who: p.solution });
  else if (p.type === 'aquarium')
    p.solution.forEach((value, tank) => apply({ type: 'level', tank, value }));
  else if (p.type === 'network')
    p.solution.forEach((v, cell) => {
      for (let k = 0; k < v; k++) apply({ type: 'rotate', cell });
    });
