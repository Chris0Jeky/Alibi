...
for (const id of E.ids)
  test(`Wrenmere: ${id} accepts its worked solution`, () => {
    assert.equal(E.check(id, W.puzzles[id].solution), true);
  });
test('The story is reachable; each activity grants points once', () => {
  let s = E.initial();
  const order = ['gate', 'shelves', 'clock', 'route', 'inference'];
  order.push('lamps', 'hanoi', 'bridges', 'magic', 'ur');
  ...
  assert.equal(E.roomStatus(s, stair).open, true);
});
...
test('Gate and shelf constraints have exactly one semantic solution', () => {
  ...
  for (let a = 0; a < 7; a++)
    for (let b = 0; b < 7; b++)
      for (let c = 0; c < 7; c++) if (E.check('gate', [a, b, c])) gate.push([a, b, c]);
  assert.deepEqual(gate, [[1, 3, 5]]);
  const permute = (xs) =>
    xs.length
      ? xs.flatMap((x, i) => permute(xs.filter((_, j) => i !== j)).map((ys) => [x, ...ys]))
      : [[]];
...
test('Unknown and nested future fields remain protected', () => {
  ...
  for (const edit of edits) {
    const s = E.initial();
    edit(s);
    assert.throws(() => E.validate(s));
  }
  const s = E.initial();
  s.completed.clock = { answer: '21:00', guided: false };
  assert.throws(() => E.validate(s));
});
test('Unfinished boards need valid shapes, not solved states', () => {
  const s = E.initial();
  s.drafts.gate = [0, 0, 0];
  s.notes = '<script>text, never markup</script>';
  assert.deepEqual(E.validate(s), s);
  s.notes = 'a'.repeat(12001);
  assert.throws(() => E.validate(s));
});
