...
with sync_playwright() as p:
    ...
    standalone.goto((ROOT/'alibi-deluxe-play.html').as_uri()+'#/home')
    standalone.wait_for_selector('.theatre-rail')
    check(standalone.locator('[data-theatre-sound],[data-theatre-ambience],[data-theatre-volume]').count()==0,'Single-file preview hides unavailable room sound controls')
    check('hosted edition' in standalone.locator('.theatre-note').inner_text() and 'Optional rain and waves download' not in standalone.locator('.theatre-note').inner_text(),'Single-file room copy explains the hosted sound boundary')
    standalone.goto((ROOT/'alibi-deluxe-play.html').as_uri()+'#/quiet/folio');standalone.wait_for_selector('.folio-intro')
