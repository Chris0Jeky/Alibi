...
with sync_playwright() as p:
 ...
 route('/play/dossier-02');action('mark','[data-cell="0"]');check(page.locator('.logic-cell.derived-mark').count()>0,'YES projects visible automatic exclusions');initial=page.evaluate('AlibiDiagnostics.getCurrent().state.marks');check(initial.count(0)==0,'Automatic exclusions are not persisted as manual crosses');action('undo');check(page.locator('.logic-cell.derived-mark').count()==0,'Undo removes automatic exclusions');action('redo');check(page.locator('.logic-cell.derived-mark').count()>0,'Redo recomputes automatic exclusions');shot('dossier-desktop')
