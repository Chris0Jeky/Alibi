...
with sync_playwright() as p:
 ...
 route('/salon/duel');check(diag()['engineLoaded'],'Games room loads its engine on demand');check(page.locator('.duel-cell.legal').count()==4,'Duel advertises four legal opening moves');duel_labels=page.locator('.duel-cell').evaluate_all("els=>els.map(e=>e.getAttribute('aria-label'))");check(all(label.startswith('Row ') and ', column ' in label for label in duel_labels),'Duel labels expose row and column');check(any('gold lantern' in label for label in duel_labels) and any('ink lantern' in label for label in duel_labels),'Duel labels expose occupied lanterns');check(any('legal move' in label for label in duel_labels),'Duel labels expose legal move state');page.locator('.duel-cell.legal').first.click();page.wait_for_function('AlibiClub.diagnostics().state.runs.duel.log.length===2');check(not diag()['botPending'],'Offline worker opponent replies');action('club-undo','[data-id="duel"]');check(len(log('duel'))==0,'Undo against keeper rewinds the whole exchange');action('club-redo','[data-id="duel"]');check(len(log('duel'))==2,'Redo restores both turns')
 ...
 route('/salon/borough');check(page.locator('.plan strong').all_text_contents().__len__()==3,'Town draft exposes three plans');borough_labels=page.locator('.borough-cell').evaluate_all("els=>els.map(e=>e.getAttribute('aria-label'))");check(all(label.startswith('Row ') and ', column ' in label for label in borough_labels),'Borough labels expose row and column');check(any('points' in label for label in borough_labels) and any('available plot' in label for label in borough_labels),'Borough labels expose occupant and legal plot state');check(len(set(page.locator('.plan strong').all_text_contents()))==3,'Opening plans are distinct choices');action('club-plan','[data-value="1"]');page.locator('.borough-cell:not(.built)').first.click();check('selected plot' in page.locator('.borough-cell.plot-selected').get_attribute('aria-label'),'Borough selected plot state is announced');check(len(log('borough'))==0,'Previewing a plot does not commit');check('points' in page.locator('[data-action="club-build"]').inner_text(),'Placement preview shows the score change');action('club-build');check(len(log('borough'))==1,'Explicit confirmation builds one tile');action('club-undo','[data-id="borough"]');check(len(log('borough'))==0,'Town undo restores draft and board');action('club-redo','[data-id="borough"]');check(len(log('borough'))==1,'Town redo restores the build')
 for k in range(17):
 ...
 route('/salon/archive');paths=['RRULL','RUDLLU','RUU','UUDRDRUULUL','RRRUUDLLDLUU','UUDDLUDLUURRRDRU','UULUURRDDDUULULDDD','DDDLLLDLLDRRULULURRRRDRU','RRRRRULLULDULURUULLDRRRRR'];directions={'R':'right','U':'up','D':'down','L':'left'}
 ...
 action('club-restart','[data-id="archive"]');check(page.locator('dialog[open]').count()==1,'Restarting the final feedback Archive room asks before clearing progress');action('club-reset-confirm');check(diag()['state']['runs']['archive']['level']==8 and len(log('archive'))==0,'Restart keeps the selected feedback room and clears only its replay');check('Every record' not in page.locator('.archive-status').inner_text(),'Restart reopens the final feedback Archive room')
 route('/play/dossier-02');page.locator('#play-context-summary').click();action('club-assist','[data-value="tidy"]');action('mark','[data-cell="0"]');check(page.locator('.logic-cell.derived-mark').count()>0,'YES projects visible automatic exclusions');initial=page.evaluate('AlibiDiagnostics.getCurrent().state.marks');check(initial.count(0)==0,'Automatic exclusions are not persisted as manual crosses');action('undo');check(page.locator('.logic-cell.derived-mark').count()==0,'Undo removes automatic exclusions');action('redo');check(page.locator('.logic-cell.derived-mark').count()>0,'Redo recomputes automatic exclusions');shot('dossier-desktop')
