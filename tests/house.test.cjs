...
const puzzle = (id, extra = {}) => ({ id, revision: 1, type: 'binary', title: id, difficulty: 'Gentle', ...extra });
const record = (p, extra = {}) => ({ key: M.key(p), puzzle: p, moves: 1, updatedAt: '2026-09-12T12:00:00Z', ...extra });
const fixtures = [puzzle('binary-01'), puzzle('old', { revision: 2, difficulty: 'Expert' }), puzzle('bridge', { type: 'bridges' })];
const route = M.locationState('#/home?ux=house&view=puzzles');
function data(extra = {}) {
  return { route, puzzles: fixtures, records: [], recent: [], games: [], first: fixtures[0], resume: null, study: M.study(), limit: 12, names: {}, icons: {}, settings: {}, media: {}, storage: 'session', ...extra };
}
test('only explicit house routes activate the review edition', () => {
  for (const hash of ['#/home', '#/library?ux=house', '#/home?ux=other', '#/play/x?ux=house']) assert.equal(M.locationState(hash).active, false);
...
test('filters round trip through URL including punctuation and unicode', () => {
  const r = { view: 'puzzles', q: 'moon & sun # été', family: 'binary', progress: 'active', level: 'Gentle' };
...
test('progress is revision-specific and completed history remains completed', () => {
  const saves = [record(puzzle('old'), { firstCompletedAt: 'yesterday' }), record(fixtures[0]), record(fixtures[2], { completedAt: 'today' })];
  assert.deepEqual(M.catalogue(fixtures, saves, { ...route, progress: 'new' }).map(M.key), ['old@2']);
  assert.deepEqual(M.catalogue(fixtures, saves, { ...route, progress: 'active' }).map(M.key), ['binary-01@1']);
  assert.deepEqual(M.catalogue(fixtures, saves, { ...route, progress: 'solved' }).map(M.key), ['bridge@1']);
});
test('all filters combine without rewriting their inputs', () => {
  const before = JSON.stringify(fixtures);
  assert.deepEqual(M.catalogue(fixtures, [], { ...route, level: 'Expert', q: 'OLD', progress: 'new' }), [fixtures[1]]);
...
test('first play avoids already completed introductory content', () => {
  ...
  assert.equal(M.firstPuzzle(fixtures, [record(fixtures[0], { completedAt: 'once' })]).id, 'bridge');
  assert.equal(M.firstPuzzle([], []), undefined);
});
test('most recent activity can be a game or a puzzle', () => {
  const records = [record(fixtures[0])];
  assert.equal(M.nextActivity(records, [{ id: 'blockcabinet', updatedAt: '2026-09-13' }]).kind, 'game');
  assert.equal(M.nextActivity(records, [{ id: 'blockcabinet', updatedAt: '2026-09-11' }]).kind, 'puzzle');
  assert.equal(M.nextActivity([], []), null);
});
test('game projection skips completed, unknown and unreadable runs without repairs', () => {
  const state = { runs: { blockcabinet: { log: [1], seed: 1 }, tictactoe: { log: [2] }, future: { log: [1] }, duel: { log: [1] } } };
  const before = JSON.stringify(state);
  const E = { blockCabinet: { replay: () => ({ done: false }) }, tictactoe: { replay: () => ({ done: true }) }, reversi: { initial: () => { throw Error('unknown'); } } };
  assert.deepEqual(M.gameRuns(state, E).map((r) => r.id), ['blockcabinet']);
...
test('every study state has a visible next step', () => {
  ...
  assert.match(M.nextStep({ ...M.study(), letter: true, visited: M.rooms.map((r) => r.id) }), /notebook/);
  assert.match(M.nextStep({ ...M.study(), solved: true }), /complete/);
});
test('imported text is escaped rather than interpreted as HTML', () => {
  const html = V.render(data({ puzzles: [puzzle('untrusted', { title: '<img src=x onerror=alert(1)>' })] }));
...
test('spoiler warning precedes the final hint and storage warning is truthful', () => {
  const html = V.render(data({ route: { ...route, view: 'notebook' }, study: { ...M.study(), hint: 2 } }));
...
test('settings keep the existing owner and canonical recovery routes', () => {
  const html = V.render(data({ route: { ...route, view: 'comfort' }, settings: { reducedMotion: true, theme: 'night' } }));
  assert.ok(html.includes('id="theme-select"'));
  assert.match(html, /data-setting="reducedMotion" checked/);
  assert.match(html, /href="#\/settings"/);
  assert.ok(V.render(data({ route: { ...route, view: 'notebook' } })).includes('#/quiet/castle/journal'));
});
test('presentation implementation does not create save writers or observers', () => {
  ...
  assert.ok(!/localStorage\.setItem|indexedDB\.open|MutationObserver|setInterval|\.innerHTML\s*=/.test(source.source));
...
test('visible navigation labels retain full accessible names', () => {
  ...
  for (const name of ['Your desk', 'Puzzles', 'The house', 'Notebook', 'Comfort']) assert.ok(html.includes(`aria-label="${name}"`));
...
test('decorative vectors use no scripts, remote dependencies or focusable targets', () => {
  ...
  for (const svg of [C.icon('house'), C.icon('<script>'), ...M.rooms.map(r=>C.engraving(r.id))]) {
...
test('filter form round-trips draft values and exposes explicit apply and reset', () => {
  const html = V.filters(data({route:{...route,q:'<unsafe>',family:'binary',level:'Gentle',progress:'active'}}));
...
test('search recognises the visible family label as well as its internal ID', () => {
  assert.equal(M.catalogue(fixtures,[],{...route,q:'SUN & MOON'}, {binary:'Sun & moon'}).length,2);
});
