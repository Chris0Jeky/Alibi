...
test('trusted catalogue validates 376 puzzles while retaining every published definition', () => {
  const catalogue = load();
  assert.equal(catalogue.puzzles.length, 376);
  assert.deepEqual(catalogue.puzzles.slice(0, original.puzzles.length), original.puzzles);
  assert.equal(new Set(catalogue.puzzles.map((p) => p.id)).size, 376);
